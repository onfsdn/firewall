...
from dp import DP
from firewall import Dyn_Fw as ip_list
import couchdb
...
class Faucet(app_manager.RyuApp):
    ...
    def reload_config(self, ev):
        ...
        if new_dp:
            ...
            #thread.start_new_thread(self.firewall)
            ryudp = self.dpset.get(new_dp.dp_id)
            self.send_flow_msgs(ryudp, flowmods)

    def firewall(self):
        # URL and Action List has to be handled
	while(1):
            whitelist_Iplist,block_iplist=ip_list.url_spamhaus()

	    for ip in whitelist_Iplist:	
                self.valve.remove_blocklist_drop_flows()     
	    for ip in block_iplist:
                self.valve.add_blocklist_drop_flows()
